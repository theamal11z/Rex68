...
export class DatabaseStorage implements IStorage {
  ...
  async getMessagesLimited(userId: string, limit: number): Promise<Message[]> {
    // This selects the latest messages first using the order by desc, then limits,
    // but returns them in ascending order for display
    const subquery = db.select().from(messages)
      .where(eq(messages.userId, userId))
      .orderBy(desc(messages.timestamp))
      .limit(limit)
      .as('recent_messages');
      
    return db.select().from(subquery).orderBy(subquery.timestamp);
  }
  ...
  // Content operations
  async getAllContents(): Promise<Content[]> {
    return db.select().from(contents).orderBy(desc(contents.timestamp));
  }

  async getContentsByType(type: string): Promise<Content[]> {
    return db.select().from(contents)
      .where(eq(contents.type, type))
      .orderBy(desc(contents.timestamp));
  }
